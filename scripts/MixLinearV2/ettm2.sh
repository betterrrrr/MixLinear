...
mkdir -p .locks
LOCK_FILE="$PROJECT_ROOT/.locks/ettm2_mixlinearv2.lock"
exec 9>"$LOCK_FILE"
if ! flock -n 9; then
    echo "Another ettm2 MixLinearV2 job is already running. lock=$LOCK_FILE"
    exit 1
fi
...
echo "Using GPU $GPU for MixLinearV2 on ETTm2"
