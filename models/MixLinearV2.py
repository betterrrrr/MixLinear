...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        # Gating mechanism
        self.gate = nn.Linear(self.enc_in * 2, self.enc_in)
    ...
    def forward(self, x):
        ...
        # Mix with Gate
        x_t_pred = x_t[:, :self.pred_len, :]
        x_f_pred = x_f[:, :self.pred_len, :]
        
        gate_input = torch.cat([x_t_pred, x_f_pred], dim=-1)
        g = torch.sigmoid(self.gate(gate_input))

        x_out = g * x_t_pred + (1 - g) * x_f_pred + seq_mean
